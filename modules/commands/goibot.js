...
module.exports.handleEvent = function({ api, event }) {
  ...
  var tl = ["chào bạn tôi là bot của H.Anh", "bạn gọi tôi có việc gì?", "tôi yêu bạn vai lon", "Yêu em <3", "Hi, chào con vợ bé:3", "Vợ gọi có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế con lợn", "Em đây~~~~", "Yêu anh Hoàng Anh nhất", "chị ấy là bae của Hoàng Anh", "Yêu thương admin nhất", "Anh ấy là phụ trợ của admin", "Sao thế công chúa", "Đừng làm em đau ~~~~", "Rên với em nhé a á á á", "Tuyển phi công nè ạ", "Làm đĩ ko ạ? dui lắm", "Cậu cô đơn ko?", "Chịch ko ạ vã quá!!!", "Được của ló :)))", "Em dthw như chủ của em ạ", "Đừng khen em ngại quá hí hí" ,"Làm chồng em ko ạ?", "Đừng spam em nha :<<, cậu chủ em mệt lắm ời", "Cút ra", "Đừng đè em mạnh!!!", "Đút tutu thôi em đau :'(", "Yêu cậu như một cực hình\nNhấp lên nhấp xuống hai mình cùng rên", "Spam cc cút", "Yêu em ko?", "Chồng em đây rồi"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  if ((event.body.toLowerCase() == "bot ngu")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: Hoàng Anh để được hỗ trợ\nLink fb nè: https://www.facebook.com/lyhoanganh.htfb", threadID, () =>
  ...
  if ((event.body.toLowerCase() == "info admin")) {
    return api.sendMessage("m muốn info admin t à? lên https://hoanganh.mx/info mà xem :33", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };

  if ((event.body.toLowerCase() == "info ad")) {
    return api.sendMessage("m muốn info admin t à? lên https://hoanganh.mx/info mà xem :33", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
  ...
  if ((event.body.toLowerCase() == "bé đã pho lâu anh chưa mà đòi xin info của anh!!") || (event.body.toLowerCase() == "bé đá pho lâu anh chưa mà đòi xin info của anh")) {
    return api.sendMessage("Anh Chỉ Cho Bé Lần Này Thôi Đấyyy (Vào Web Để Xem : https://hoanganh.mx/info)", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "dạ") || (event.body.toLowerCase() == "da")) {
    return api.sendMessage("Ngoan đấy", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "anh Hoàng Anh ") || (event.body.toLowerCase() == "anh Hoang Anh")) {
  ...
  if ((event.body.toLowerCase() == "a Hoang Ann") || (event.body.toLowerCase() == "a Hoang Anh")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };

  if ((event.body.toLowerCase() == "H.Anh") || (event.body.toLowerCase() == "H.Anh")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };

  if ((event.body.toLowerCase() == "@Hoàng Anh") || (event.body.toLowerCase() == "@Hoàng Anh")) {
    return api.sendMessage("Ai kêu chủ tao đấy, mà làm ơn đừng tag anh ấy mà hãy liên hệ qua Fb: https://www.facebook.com/lyhoanganh.htfb ", threadID);
  };
 ...
 if ((event.body.toLowerCase() == "bsvv nha mng") || (event.body.toLowerCase() == "bsvv nha mng")) {
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin tao Lý Hoàng Anh có gì dùng lệnh -ad hoặc -adm để biết thêm chi tiết", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "cặc") || (event.body.toLowerCase() == "cặc")) {
    return api.sendMessage("️Văn minh chút đi bạn ơi lớn rồi ", threadID);
  };

  if ((event.body.toLowerCase() == "ông Hoàng Anh ") || (event.body.toLowerCase() == "ong Hoang Anh ")) {
    return api.sendMessage("️Kêu chủ của tôi gì á :)?", threadID);
  };

  if ((event.body.toLowerCase() == "ôg Hoàng Anh ") || (event.body.toLowerCase() == "og Hoang Anh ")) {
